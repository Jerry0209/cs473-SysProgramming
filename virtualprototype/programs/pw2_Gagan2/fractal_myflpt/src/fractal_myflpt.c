...
int low = 0, high = 31, top_bit = -1;
while (low <= high) {
    int mid = (low + high) / 2;
    unsigned int f = 1u << mid;

    if (mag >= f) {
        top_bit = mid;     // mid bit is set or lower bits might be set
        low = mid + 1;     // search higher bits
    } else {
        high = mid - 1;    // search lower bits
    }
}
...
mfp mfp_mul(mfp a, mfp b) {
  ...
  int low = 0, high = 31, highest = -1;
while (low <= high) {
    int mid = (low + high) / 2;
    unsigned int f = 1u << mid;

    if (mag >= f) {
        highest = mid;     // mid bit is set or lower bits might be set
        low = mid + 1;     // search higher bits
    } else {
        high = mid - 1;    // search lower bits
    }
...
// 'highest' now holds the index of the highest set bit (or -1 if mag == 0)
